...
my ($Queue, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub,$Resource,$Job_prefix,$Workdir,$Verbose, $Help);
GetOptions(
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
	"interval:i"=>\$Interval,
...
$Maxjob ||= 30;
$Convert ||= 'yes';
$Resource ||= "nodes=1:ppn=1,mem=2G,walltime=200:00:00";
$Job_prefix ||= "work";
$Workdir ||= `pwd`;
...
while (@Shell) {
	...
	my $job_cmd = "qsub ";  ## -l h_vmem=16G,s_core=8
        $job_cmd .= "-d $Workdir " if (defined $Workdir); ## set working direcotry for qsub, some tools will output file there 
		...
		while (1) {
			...
			if ($i < $Maxjob || ($run_num != -1 && $run_num < $Maxjob) ) {
				my $job_return = `$job_cmd -V $Shell[$i]`;
                                print "$job_return\n";
				my $job_id = $1 if($job_return =~ /(\d+)\.torque/); #300021.torque01.int.bioinfo.ucr.edu
